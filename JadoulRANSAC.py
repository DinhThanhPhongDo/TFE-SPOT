...
def ransac(pts,pts_n, thresh_d=0.05,thresh_n=0.8,epoch=1000, tqdm_bool=False) :
    ...
    """
    n_pts    = len(pts)
    epoch = int(np.log(1-0.95)/np.log(1-0.05**3))
    print(epoch)
...
def get_all_planes(pcd, voxel_size = 0.1, n_inliers = 100, n_plane=1, display = False,tqdm_bool=False):
    ...
    while(len(planes)<n_plane and len(pcd_plane.points) > 3) :
        ...
        plane,inliers= ransac(pts,pts_n, thresh_d=0.05,thresh_n=0.8,epoch=1000, tqdm_bool=tqdm_bool)
        ...
        print(len(inlier_cloud.points)/len(outlier_cloud.points))
...
if __name__ =='__main__':
    ...
    get_all_planes(pcd,n_inliers=500, n_plane=20, display = True,tqdm_bool=True)
