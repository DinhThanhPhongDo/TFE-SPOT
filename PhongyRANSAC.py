...
def get_all_planes(pts,pts_n,n_plane,i_plane=0):
    ...
    plane,inlier_mask= ransac(pts,pts_n,thresh_n=0.9,thresh_d=0.1,epoch=5000)
...
if __name__ =='__main__':
    pts = o3d.io.read_point_cloud(r'hough-plane-python-master\RES\10.pcd')
    pts = pts.voxel_down_sample(voxel_size=0.1)
    pts.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.2, max_nn=30))
    pts_n = np.asarray(pts.normals)
    pts= np.asarray(pts.points)

    get_all_planes(pts,pts_n,n_plane=20)
